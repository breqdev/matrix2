function Fish(id,w,h){
	...
	this.save = function(){
		if(typeof Blob !== "function" || typeof document === "undefined" || typeof window === "undefined"){
	...
	this.draw = function(){
		...
		this.paper.path(path).attr({'stroke':'none','fill':colours[this.colour].bg,'clip-path':'shape'});
		...
		this.paper.circle(eye.x, eye.y, 0.02).attr({'stroke':'none','fill':'white'}).transform(this.transform);
		this.paper.circle(eye.x, eye.y, 0.015).attr({'stroke':'none','fill':'black'}).transform(this.transform);
		this.paper.draw();
		...
		if(this.saveable && typeof getEl === "function"){
			...
			if(saveEl) saveEl.innerHTML = '<span class="line">S</span>ave SVG ('+niceSize(this.svg.length)+')';
		}
...
function SVG(id, width, height){
	...
	this.canvas.innerHTML = '<svg height="'+this.h+'" version="1.1" width="'+this.w+'" viewBox="0 0 '+this.w+' '+this.h+'" xmlns="http://www.w3.org/2000/svg"><desc>Created by stuQuery SVG</desc></svg>';
...
SVG.prototype.draw = function(){
	let dom = '<svg height="'+this.h+'" version="1.1" width="'+this.w+'" viewBox="0 0 '+this.w+' '+this.h+'" xmlns="http://www.w3.org/2000/svg">';
	let i,j,a;
		...
		for(i = 0; i < this.clippaths.length; i++){
			...
			if(this.clippaths[i].type){
				...
				for(j in this.clippaths[i]){
					if(j != "type" &&  j != "id" && typeof this.clippaths[i][j]!=="object" && typeof this.clippaths[i][j]!=="function" && j != "attributes"){
