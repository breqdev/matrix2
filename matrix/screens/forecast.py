...
class Forecast(Screen[ForecastData | None]):
    ...
    def get_image_64x64(self) -> Image.Image:
        ...
        # Top bar
        draw.text((1, 1), "3-Day", font=font, fill=TIME_DATE_COLOR)
    ...
    def get_image_64x32(self) -> Image.Image:
        ...
        for i, day_index in enumerate([1, 2]):
            ...
            draw.text((1, y + 1), day_label, font=font, fill=TIME_DATE_COLOR)
            ...
            if icon_name is not None:
                icon = Image.open(Path.cwd() / "icons" / "weather" / "16px" / f"{icon_name}.png")
                image.paste(icon, (23, y + 0))

            draw.line((42, y + 4, 44, y + 2, 46, y + 4), fill=HIGH_COLOR)
            draw.text((49, y + 1), f"{temp_max_f}°", font=font, fill=HIGH_COLOR)
            draw.line((42, y + 11, 44, y + 13, 46, y + 11), fill=LOW_COLOR)
            draw.text((49, y + 9), f"{temp_min_f}°", font=font, fill=LOW_COLOR)
